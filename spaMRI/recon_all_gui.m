...
for i=1:length(subjects_name)
    ...
    if get(handles.checkbox2,'value')
        if openmp <= 0
            ...
            commands         = ['recon-all -s ',subjects_name{i},' -autorecon1 -no-isrunning ',' -parallel -openmp ',parallel,';'];
        end
        fs_run_unix(commands);
        clear commands
    end
    ...
    if get(handles.checkbox3,'value')
        if openmp <= 0
            ...
            commands         = ['recon-all -s ',subjects_name{i},' -autorecon2 -no-isrunning ',' -parallel -openmp ',parallel,';'];
        end
        fs_run_unix(commands);
        clear commands
    end
    ...
    if get(handles.checkbox4,'value')
        if openmp <= 0
            ...
            commands         = ['recon-all -s ',subjects_name{i},' -autorecon3 -no-isrunning ',' -parallel -openmp ',parallel,';'];
        end
        fs_run_unix(commands);
        clear commands
    end
